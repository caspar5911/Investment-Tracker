...
from investment_tracker.quant.phase4.gate3.models import ArtifactIdentity, DataPartitionIdentity, Gate3AuthorityError, SafetyState
...
from investment_tracker.quant.phase4.gate3_campaign.dependencies import EXECUTION_CONTENT
from investment_tracker.quant.phase4.gate3_campaign.result_methodology import preflight_result_schema
...
from .dependencies import RESULT_SCHEMA_CONTENT, SCORED_PANEL_SHA256, load_runner_dependencies


SPEC_PATH = "docs/superpowers/specs/2026-09-16-phase-4-gate-3-campaign-orchestrator-design.md"
SPEC_CONTENT_SHA256 = "6d4c2ba976bf5d388ea5bf490bac1e441d0256fc6b786c94a0292e2feebae3c3"
SOURCE_FILES = tuple(
    sorted(
...
RESULT_SCHEMA_MANIFEST = ArtifactIdentity(
    kind="gate3_result_schema_manifest",
    content_sha256=RESULT_SCHEMA_CONTENT,
    path=(
        "results/phase4/gate3/campaign_result_schema/"
        "gate3_result_schema_manifest/sha256/"
        f"{RESULT_SCHEMA_CONTENT}/manifest.json"
    ),
    sha256="0d2fc3534251dcefacc3c13f89b457ac4c261fc6e2be261229becd9bb6b71bb6",
)
RESULT_SCHEMA_CONTRACT = ArtifactIdentity(
    kind="result_schema_contract",
    content_sha256="efd5b3a4eb4d77d2cf6f68ad875376b75443e642e3439d698035a3253a3c0979",
    path=(
        "results/phase4/gate3/campaign_result_schema/result_schema_contract/sha256/"
        "efd5b3a4eb4d77d2cf6f68ad875376b75443e642e3439d698035a3253a3c0979/contract.json"
    ),
    sha256="0dc4c3db90ed2ffa688c08dd2431439bbdb4ce6f2c5225504117a314dfc1075f",
)
...
class RunnerManifest(FrozenGate2Model):
    ...
    attempt_schema: Literal["PHASE4-GATE3-RUNNER-ATTEMPT-v1"] = "PHASE4-GATE3-RUNNER-ATTEMPT-v1"
    receipt_schema: Literal["PHASE4-GATE3-RUNNER-RECEIPT-v1"] = "PHASE4-GATE3-RUNNER-RECEIPT-v1"
    result_set_schema: Literal["PHASE4-GATE3-CAMPAIGN-RESULT-SET-v1"] = (
        "PHASE4-GATE3-CAMPAIGN-RESULT-SET-v1"
    )
    run_entrypoint: Literal["EXPLICIT_HASH_PREFLIGHT_GATED"] = "EXPLICIT_HASH_PREFLIGHT_GATED"
...
def spec_identity(root: Path) -> ArtifactIdentity:
    ...
    path = contained_path(repository, tuple(SPEC_PATH.split("/")), code="RUNNER_SPEC_MISMATCH")
...
class RunnerAuthorityStore:
    ...
    def write(self, document: dict[str, object]) -> ArtifactIdentity:
        ...
        handle, name = tempfile.mkstemp(prefix=".tmp-gate3-runner-seal-", dir=path.parent)
        temporary = Path(name)
        try:
    ...
    def read(self, identity: ArtifactIdentity) -> dict[str, object]:
        if identity.kind != "gate3_campaign_runner_manifest":
            raise ValueError("RUNNER_MANIFEST_MISSING")
        path = self._path(identity.content_sha256)
        expected_path = normalize_repository_path(self.root, path)
...
def declared_manifest(
    ...
    if preflight_gate3(root, CORRECTED_GATE3_MANIFEST.content_sha256).gate3 != "READY":
        raise ValueError("RUNNER_DEPENDENCY_MISMATCH")
    if preflight_execution_methodology(root, EXECUTION_CONTENT).gate3 != "GATE3_CAMPAIGN_READY_TO_EXECUTE":
        raise ValueError("RUNNER_DEPENDENCY_MISMATCH")
    if preflight_result_schema(root, RESULT_SCHEMA_CONTENT).status != "GATE3_CAMPAIGN_RESULT_SCHEMA_SEALED":
        raise ValueError("RUNNER_DEPENDENCY_MISMATCH")
    return RunnerManifest(
        ...
        result_schema_manifest=RESULT_SCHEMA_MANIFEST,
        result_schema_contract=RESULT_SCHEMA_CONTRACT,
...
def validate_manifest(root: Path, manifest: RunnerManifest, *, require_source: bool = True) -> None:
    ...
    if require_source:
        ...
        if manifest.source_bundle is None or manifest.source_bundle.producing_revision != manifest.source_revision:
            raise ValueError("RUNNER_SOURCE_MISMATCH")
        _validate_source_revision(repository, manifest.source_revision)
        expected_bundle = source_bundle_identity(repository, manifest.source_revision, SOURCE_FILES)
...
def _load_manifest(root: Path, digest: str) -> tuple[ArtifactIdentity, RunnerManifest]:
...
def preflight_runner(root: Path, manifest_content_sha256: str) -> RunnerPreflight:
