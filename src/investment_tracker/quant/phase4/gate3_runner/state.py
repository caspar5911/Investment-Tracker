...
class RunnerStateStore:
    ...
    def _position_path(self, kind: Literal["attempt", "receipt"], position: int) -> Path:
    ...
    def _identity(self, kind: str, path: Path, payload: bytes) -> ArtifactIdentity:
    ...
    def _publish(self, path: Path, payload: bytes) -> None:
        ...
        if checked != path:
            raise ValueError("ARTIFACT_PATH_INVALID")
        handle, name = tempfile.mkstemp(prefix=".tmp-gate3-runner-", dir=path.parent)
        temporary = Path(name)
        try:
    ...
    def _read_optional(self, path: Path, model: type[T], kind: str) -> tuple[T, ArtifactIdentity] | None:
        if path.is_symlink():
            raise ValueError("ARTIFACT_PATH_INVALID")
        if not path.exists():
            return None
    ...
    def read_attempt(self, position: int) -> tuple[AttemptRecord, ArtifactIdentity] | None:
        path = self._position_path("attempt", position)
        return self._read_optional(path, AttemptRecord, "phase4_gate3_runner_attempt")
    ...
    def read_receipt(self, position: int) -> tuple[PositionReceipt, ArtifactIdentity] | None:
        path = self._position_path("receipt", position)
        return self._read_optional(path, PositionReceipt, "phase4_gate3_runner_receipt")
    ...
    def write_result_set(self, result_set: CampaignResultSet) -> ArtifactIdentity:
        ...
        self._publish(path, payload)
        return self._identity("phase4_gate3_campaign_result_set", path, payload)

    def read_result_set(self) -> tuple[CampaignResultSet, ArtifactIdentity] | None:
        path = self._result_set_path()
        return self._read_optional(path, CampaignResultSet, "phase4_gate3_campaign_result_set")
