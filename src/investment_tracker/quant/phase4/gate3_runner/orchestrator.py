...
from investment_tracker.quant.phase4.gate3_campaign.result_artifacts import (
    ResultArtifactStore,
)
...
def _validate_population(context: RunnerDependencies) -> None:
    ...
    if (
        len(bindings) != 180
        or tuple(binding.budget_position for binding in bindings)
        != tuple(range(1, 181))
...
def generate_targets(
    ...
    return tuple(
        generate_target(
            context.campaign.gate2,
            binding,
            context.market_input,
            offset,
        )
...
def _failure_reason(exc: Exception) -> str:
    ...
    if token and all(
        ch.isupper() or ch.isdigit() or ch == "_"
        for ch in token
    ):
        return token
...
def _observe(
    ...
    next_streak, should_stop = update_oos_failure_streak(
        current_streak,
        outcome,
    )
    if (
        outcome.benchmark_excess_return is not None
...
def run_campaign(
    ...
    *,
    ...
    stops: dict[
        str,
        tuple[int, tuple[ArtifactIdentity, ...]],
    ] = {}
    ...
    for binding in context.campaign.bindings:
        ...
        if existing is not None:
            ...
            if result.status == "CAMPAIGN_EXECUTION_FAILED":
                raise CampaignExecutionError(
                    ...
                    f"at position {position}: {result.reason}"
                )
            if result.status == "SKIPPED_FAMILY_STOP":
        ...
        if stop is not None:
            ...
            result_identity = results.write_result(
                skipped,
                context.result_authority,
            )
            state.write_receipt(
        ...
        expected_attempt = _attempt_record(
            binding,
            runner_manifest,
        )
        ...
        try:
            ...
            if (
                ...
                raise ValueError(
                    "RUNNER_RESULT_POSITION_MISMATCH"
                )
            if result.status == "SKIPPED_FAMILY_STOP":
                raise ValueError(
                    "RUNNER_EVALUATOR_CANNOT_INVENT_SKIP"
                )
            result_identity = results.write_result(
                result,
                context.result_authority,
            ...
            try:
                failure_identity = results.write_result(
                    failure,
                    context.result_authority,
                )
                state.write_receipt(
                ...
                raise CampaignExecutionError(
                    "CAMPAIGN_EXECUTION_FAILED_UNPUBLISHED "
                    f"at position {position}"
                ) from publication_error
            raise CampaignExecutionError(
                "CAMPAIGN_EXECUTION_FAILED "
                f"at position {position}: {failure.reason}"
            ) from exc
    ...
    aggregate = canonical_sha256(
        {
            "schema_version": (
                "PHASE4-GATE3-CAMPAIGN-RESULT-SET-IDENTITY-v1"
            ),
            "runner_manifest": runner_manifest.model_dump(
                mode="json"
            ),
            "results": [
    ...
    result_set_identity = state.write_result_set(
        result_set
    )
