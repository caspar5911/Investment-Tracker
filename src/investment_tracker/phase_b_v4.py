"""Strict Candidate-v4 Phase-B acceptance, frozen before v4 panel history."""
...
from .candidate_v4 import V4_VALIDATION_PANEL
from .governance import assert_symbol_allowed

PHASE_B_V4_VERSION="PHASEB-v3.0"
MIN_MATURED_20D=20
MIN_ADEQUATELY_SAMPLED_PROXIES=4
MIN_EPISODES_PER_PROXY=2
...
def _median(values):
    rows=sorted(values)
    if not rows: raise ValueError("median requires values")
    m=len(rows)//2
...
def _asset(value:str)->str:
    ...
    if normalized not in V4_VALIDATION_PANEL:
        raise ValueError(f"asset outside Candidate-v4 panel: {normalized}")
    return normalized


def evaluate_phase_b_v4(
    ...
    if sample:
        return Report(PHASE_B_V4_VERSION,Status.INCONCLUSIVE,tuple(sample),len(r20),len(r60),len(adequate),nonneg,m20s,m20c,m60,mnet,mcash,mdip)
    failures=[]
    ...
    return Report(PHASE_B_V4_VERSION,Status.FAIL if failures else Status.PASS,tuple(failures),len(r20),len(r60),len(adequate),nonneg,m20s,m20c,m60,mnet,mcash,mdip)
