...
RESULT_SCHEMA = "SUCCESSOR-PHASE6-FINAL-HOLDOUT-EVALUATION-v1"
...
def _statement_amount(statement: object) -> float:
    ...
    if len(unique) != 1:
        raise ValueError("SUCCESSOR_PHASE6_DIVIDEND_AMOUNT_AMBIGUOUS")
    return unique[0]
...
def _corporate_actions(
    ...
    for row_index, row in rehab.iterrows():
        ...
        if cash_components or endpoint:
            ...
            statement_amounts = [
                _statement_amount(item.get("statement")) for item in endpoint
            ]
            if not math.isclose(
                sum(statement_amounts),
                sum(amount for _, amount in cash_components),
                rel_tol=1e-6,
                abs_tol=1e-8,
            ):
                raise ValueError(
                    f"SUCCESSOR_PHASE6_DIVIDEND_AMOUNT_RECONCILIATION_FAILED:{symbol}:{ex_date.date()}"
                )
