...
from investment_tracker.quant.phase4.gate3_runner.orchestrator import CampaignExecutionError, run_campaign
...
def manifest_identity() -> ArtifactIdentity:
    content = "5" * 64
    path = f"results/phase4/gate3/campaign_runner/runner_manifest/sha256/{content}/manifest.json"
...
def test_unavailable_oos_streak_stops_only_54_member_families_and_accounts_all_positions(context, tmp_path):
    calls: list[int] = []
...
def test_system_evaluation_failure_is_published_and_fails_closed(context, tmp_path):
...
def test_runner_source_contains_no_selection_or_provider_surface():
    import investment_tracker.quant.phase4.gate3_runner.orchestrator as module

    source = inspect.getsource(module)
    assert "select_survivor" not in source
    assert "moomoo" not in source.lower()
